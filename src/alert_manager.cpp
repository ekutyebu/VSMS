...
void AlertManager::update(const SensorData& sensorData) {
    ...
    if (newLevel != currentLevel) {
        ...
        // If critical, trigger SMS alert (once per event)
        if (currentLevel == STATUS_CRITICAL) {
            if (!smsSentForCurrentEvent) {
                sendCriticalSMS(sensorData);
...
void AlertManager::sendCriticalSMS(const SensorData& d) {
    char smsText[160];
    sprintf(smsText, 
    ...
    if (gsmOnline) {
        Serial.println("[AlertManager] Directing SIM800L to send SMS...");
        
        char phoneCmd[40];
        sprintf(phoneCmd, "AT+CMGS=\"%s\"", EMERGENCY_PHONE_NUMBER);
